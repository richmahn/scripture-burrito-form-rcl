...
import { uiSchemas } from '../core/uiSchemas';
...
export default function Playground({ themes, validators }: PlaygroundProps) {
  ...
  const [schema, setSchema] = useState<RJSFSchema>(schemas.SBv100.schema as RJSFSchema);
  const [uiSchema, setUiSchema] = useState<UiSchema>(uiSchemas.ScriptureBurrito);
  const [formData, setFormData] = useState<any>(examples.TextStories);
  ...
  const load = useCallback(
    (data: any) => {
      ...
      const { uiSchema = {}, formData, theme: dataTheme = theme } = data;
